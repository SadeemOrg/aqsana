...
<div   wire:poll.75ms  class="flex ">
    ...
    <div x-data="{ dropdownOpen: false }" class="relative ">
        ...
        <button  @click="dropdownOpen = !dropdownOpen" class=" -top-1.5  relative z-10 block rounded-md bg-white p-2 focus:outline-none">
            <strong class="relative inline-flex items-center rounded  border-gray-200 px-2.5 py-1.5 text-xs font-medium">
                @if (! $count == 0)
                <span class="text-white absolute -top-1 -right-0 h-5 w-5 rounded-full bg-red-600 flex justify-center items-center items">
                    <span>{{ $count }}</span></span>

                    @endif
                  <svg  class=" ml-1.5  h-7 w-7 text-gray-800" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 20 20" fill="currentColor">
        ...
        <div x-show="dropdownOpen" class="absolute right-0 mt-2 bg-white rounded-md shadow-lg overflow-hidden z-20" style="width:20rem;">
            ...
            @endphp
                ...
                <a href=""  class="flex justify-end   px-4 py-3 border-b hover:bg-gray-100  @if ($notification->read_at == null)
                bg-indigo-300
                @endif ">
                  <div>
            ...
            <a href="/Admin/notification" class="block bg-gray-800 text-white text-center font-bold">See all notifications</a>
            @else
