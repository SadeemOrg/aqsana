    ...
    <div
        ...
        <div dir="rtl" class=" relative shadow-sm  w-full bg-white border-0 h-full  rounded-md">
            <form class="h-full w-full" action="{{ route('pagesearch') }}" method="get">
                ...
                <div class="absolute inset-y-0 left-6 sm:flex items-center hidden">
                    <!-- Heroicon name: solid/mail -->
                    <button type="submit"
                ...
                <svg onclick="location.href='{{ route('pagesearch') }}'"
                    class="svgSearch block absolute sm:hidden top-6 right-4 " width="20" height="20" viewBox="0 0 30 30"
                ...
                <div
                    class="card absolute bg-green-50 search-card z-10 my-2 min-h-[50px] h-auto max-h-40 overflow-y-scroll w-full lg:w-[100%] top-[100%] right-0 rounded shadow hidden">
                    <div class="card-body p-3 text-right search-result-box flex flex-col items-start justify-start gap-x-2 gap-y-3"
                        style="">
                    </div>
                </div>
            </form>
