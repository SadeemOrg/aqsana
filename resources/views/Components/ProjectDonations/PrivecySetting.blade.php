...
@php
$CancellationPolicy_AR_text = "1. يحق للشخص الذي يقوم بالتبرع التواصل مع الجمعية لطلب تغيير تفاصيل الدفع وإلغاء
المعاملة.</br>
...
$CancellationPolicy_HE_text = "1. מבצעת הפעולה תהיה רשאית לפנות לעמותה בבקשה לשינוי פרטי החיוב וביטול עסקה </br>
...
$privacyTerms_Ar_Text = " 1. يجوز لجمعية الأقصى استخدام أموال التبرع لصالح قضيتها ووفقا لتقديرها.</br>
...
<a class='text-[#349A37] text-sm sm:text-lg underline' href='/contact-us'>فاتصل بنا </a>
...
$privacyTerms_HE_Text = "1. עמותת אלאקסא רשאית להשתמש בכספי התרומה, לטובת עילותה ולפי שיקול דעתה. </br>
...
אינך
מסכימ/ה לכך,
<a class='text-green-900 text-lg sm:text-2xl underline' href='/contact-us'>צרו עמנו קשר </a>
...
$CancellationPolicy_ar = nova_get_setting('Cancellation_Policy_ar', $CancellationPolicy_AR_text);
$CancellationPolicy_he = nova_get_setting('Cancellation_Policy_he', $CancellationPolicy_HE_text);

$privacyTerms_ar = nova_get_setting('privacy_terms_ar', $privacyTerms_Ar_Text);
$privacyTerms_he = nova_get_setting('privacy_terms_he', $privacyTerms_HE_Text);
...
<div dir="rtl" class="PrivecySettingModal relative hiddenModal  z-10" aria-labelledby="modal-title" role="dialog"
    ...
    <div class="fixed z-10 inset-0 sm:top-14 overflow-y-auto ">
        <div
            ...
            <div
                ...
                <div
                    class="flex flex-col items-start justify-start tab tab-1 font-Flatnormal max-w-md h-full sm:max-w-xl">
                    <P class="pt-2  text-xs sm:text-base">
                        {!! $CancellationPolicy_ar !!}
                    </P>
                    <!--Cancellation Policy -->
                    <h4 class="text-2xl mt-6 leading-6 font-medium text-[#349A37] tab tab-1" id="modal-title">מדיניות
                        ביטולים
                    </h4>
                    <P class="pt-2 tab tab-1 text-sm sm:text-lg font-medium">
                        {!! $CancellationPolicy_he !!}
                    </P>
                </div>
                <!---Privacy Policy -->
                <div
                    class=" flex flex-col items-start justify-start tab font-Flatnormal tab-2 max-w-md md:max-w-2xl h-full">
                    <p class="font-FlatBold  sm:text-base pb-2">
                    ...
                    <P class="pt-4 font-Flatnormal text-sm sm:text-lg tab tab-2 h-full">
                        {!! $privacyTerms_he !!}
                    </P>
                </div>
            </div>
