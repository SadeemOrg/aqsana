<style>
    .newsShaddow:hover {
        box-shadow: 0px 0px 1px 0px #0000000A;
        box-shadow: 0px 2px 6px 0px #0000000A;
        box-shadow: 0px 16px 24px 0px #0000000F;
    }
</style>
<div  class="max-w-7xl mx-auto px-4 mt-12 md:mt-24 sm:px-6 lg:px-8">
    <div class="relative mt-0 xl:mt-8">
        <p class="font-FlatBold text-3xl text-center xl:text-right"> اهدافنا </p>
        <div class="absolute border-b-[4px] pt-2 border-b-[#349A37] w-7 hidden xl:block">
        </div>
    </div>
    <div class="pt-6 md:pt-16 grid grid-cols-1 lg:grid-cols-2 gap-y-3 md:gap-y-6 gap-x-6">
        <!--First section -->
        <div
            class="newsShaddow flex flex-row justify-between py-6 items-center w-full text-[15px] sm:text-lg font-FlatBold text-right text-[#101426]   bg-white md:px-1 xl:px-4">
            <p class="max-w-[475px] ">
                مسح وأرشفة المواقع والأراضي والممتلكات الوقفية في فلسطين 1948 وبيان إحداثياتها....
            </p>
            <svg class="hidden sm:block" width="11" height="17" viewBox="0 0 11 17" fill="none"
                xmlns="http://www.w3.org/2000/svg">
                <path
                    d="M0.654771 8.49998C0.654771 8.19531 0.771103 7.89067 1.00328 7.65839L8.31297 0.348771C8.77795 -0.116216 9.53185 -0.116216 9.99665 0.348771C10.4614 0.81357 10.4614 1.56732 9.99665 2.03234L3.52863 8.49998L9.99642 14.9677C10.4612 15.4326 10.4612 16.1863 9.99642 16.6511C9.53162 17.1163 8.77773 17.1163 8.31274 16.6511L1.00305 9.34158C0.770839 9.10918 0.654771 8.80454 0.654771 8.49998Z"
                    fill="#349A37" />
            </svg>
        </div>
        <!--second section -->
        <div
            class="newsShaddow flex flex-row justify-between py-6 items-center w-full text-[15px] sm:text-lg font-FlatBold text-right text-[#101426]   bg-white md:px-1 xl:px-4">
            <p class="max-w-[475px] ">
                بناء تصور وخطة تدخل للدفاع عن هذه الممتلكات والمقدسات قضائياً ومن خلال ....
            </p>
            <svg class="hidden sm:block" width="11" height="17" viewBox="0 0 11 17" fill="none"
                xmlns="http://www.w3.org/2000/svg">
                <path
                    d="M0.654771 8.49998C0.654771 8.19531 0.771103 7.89067 1.00328 7.65839L8.31297 0.348771C8.77795 -0.116216 9.53185 -0.116216 9.99665 0.348771C10.4614 0.81357 10.4614 1.56732 9.99665 2.03234L3.52863 8.49998L9.99642 14.9677C10.4612 15.4326 10.4612 16.1863 9.99642 16.6511C9.53162 17.1163 8.77773 17.1163 8.31274 16.6511L1.00305 9.34158C0.770839 9.10918 0.654771 8.80454 0.654771 8.49998Z"
                    fill="#349A37" />
            </svg>
        </div>
        <!--third section -->
        <div
            class="newsShaddow flex flex-row justify-between py-6 items-center w-full text-[15px] sm:text-lg font-FlatBold text-right text-[#101426]   bg-white md:px-1 xl:px-4">
            <p class="max-w-[475px] ">
                رصد ومتابعة قضايا المقدسات الإسلامية في الداخل الفلسطيني ومدينة القدس ....
            </p>
            <svg class="hidden sm:block" width="11" height="17" viewBox="0 0 11 17" fill="none"
                xmlns="http://www.w3.org/2000/svg">
                <path
                    d="M0.654771 8.49998C0.654771 8.19531 0.771103 7.89067 1.00328 7.65839L8.31297 0.348771C8.77795 -0.116216 9.53185 -0.116216 9.99665 0.348771C10.4614 0.81357 10.4614 1.56732 9.99665 2.03234L3.52863 8.49998L9.99642 14.9677C10.4612 15.4326 10.4612 16.1863 9.99642 16.6511C9.53162 17.1163 8.77773 17.1163 8.31274 16.6511L1.00305 9.34158C0.770839 9.10918 0.654771 8.80454 0.654771 8.49998Z"
                    fill="#349A37" />
            </svg>
        </div>
        <!--fourth section -->
        <div
            class="newsShaddow flex flex-row justify-between py-6 items-center w-full text-[15px] sm:text-lg font-FlatBold text-right text-[#101426]   bg-white md:px-1 xl:px-4">
            <p class="max-w-[475px] ">
                نشر الوعي وتصليط الضوء على واقع الانتهاكات المستمرة ومحاولات السيطرة ....
            </p>
            <svg class="hidden sm:block" width="11" height="17" viewBox="0 0 11 17" fill="none"
                xmlns="http://www.w3.org/2000/svg">
                <path
                    d="M0.654771 8.49998C0.654771 8.19531 0.771103 7.89067 1.00328 7.65839L8.31297 0.348771C8.77795 -0.116216 9.53185 -0.116216 9.99665 0.348771C10.4614 0.81357 10.4614 1.56732 9.99665 2.03234L3.52863 8.49998L9.99642 14.9677C10.4612 15.4326 10.4612 16.1863 9.99642 16.6511C9.53162 17.1163 8.77773 17.1163 8.31274 16.6511L1.00305 9.34158C0.770839 9.10918 0.654771 8.80454 0.654771 8.49998Z"
                    fill="#349A37" />
            </svg>
        </div>
        <!--5th section -->
        <div
            class="newsShaddow flex flex-row justify-between py-6 items-center w-full text-[15px] sm:text-lg font-FlatBold text-right text-[#101426]   bg-white md:px-1 xl:px-4">
            <p class="max-w-[475px] ">
                إثبات مرجعية الأوقاف للمسلمين وتشكيل أداة إضافية لدحض الوهم والادعاء المزعوم ....
            </p>
            <svg class="hidden sm:block" width="11" height="17" viewBox="0 0 11 17" fill="none"
                xmlns="http://www.w3.org/2000/svg">
                <path
                    d="M0.654771 8.49998C0.654771 8.19531 0.771103 7.89067 1.00328 7.65839L8.31297 0.348771C8.77795 -0.116216 9.53185 -0.116216 9.99665 0.348771C10.4614 0.81357 10.4614 1.56732 9.99665 2.03234L3.52863 8.49998L9.99642 14.9677C10.4612 15.4326 10.4612 16.1863 9.99642 16.6511C9.53162 17.1163 8.77773 17.1163 8.31274 16.6511L1.00305 9.34158C0.770839 9.10918 0.654771 8.80454 0.654771 8.49998Z"
                    fill="#349A37" />
            </svg>
        </div>
        <!--6th section -->
        <div
            class="newsShaddow flex flex-row justify-between py-6 items-center w-full text-[15px] sm:text-lg font-FlatBold text-right text-[#101426]   bg-white md:px-1 xl:px-4">
            <p class="max-w-[475px] ">
                إعمار المسجد الأقصى وشد الرحال إليه وتسيير قوافل المصلين(قوافل الأقصى) .....
            </p>
            <svg class="hidden sm:block" width="11" height="17" viewBox="0 0 11 17" fill="none"
                xmlns="http://www.w3.org/2000/svg">
                <path
                    d="M0.654771 8.49998C0.654771 8.19531 0.771103 7.89067 1.00328 7.65839L8.31297 0.348771C8.77795 -0.116216 9.53185 -0.116216 9.99665 0.348771C10.4614 0.81357 10.4614 1.56732 9.99665 2.03234L3.52863 8.49998L9.99642 14.9677C10.4612 15.4326 10.4612 16.1863 9.99642 16.6511C9.53162 17.1163 8.77773 17.1163 8.31274 16.6511L1.00305 9.34158C0.770839 9.10918 0.654771 8.80454 0.654771 8.49998Z"
                    fill="#349A37" />
            </svg>
        </div>
        <!--7th section -->
        <div
            class="newsShaddow flex flex-row justify-between py-6 items-center w-full text-[15px] sm:text-lg font-FlatBold text-right text-[#101426]   bg-white md:px-1 xl:px-4">
            <p class="max-w-[475px] ">
                رفع مستوى الوعي وتعزيز الانتماء لدى أبناء وبنات الداخل الفلسطيني ومدينة القدس ....
            </p>
            <svg class="hidden sm:block" width="11" height="17" viewBox="0 0 11 17" fill="none"
                xmlns="http://www.w3.org/2000/svg">
                <path
                    d="M0.654771 8.49998C0.654771 8.19531 0.771103 7.89067 1.00328 7.65839L8.31297 0.348771C8.77795 -0.116216 9.53185 -0.116216 9.99665 0.348771C10.4614 0.81357 10.4614 1.56732 9.99665 2.03234L3.52863 8.49998L9.99642 14.9677C10.4612 15.4326 10.4612 16.1863 9.99642 16.6511C9.53162 17.1163 8.77773 17.1163 8.31274 16.6511L1.00305 9.34158C0.770839 9.10918 0.654771 8.80454 0.654771 8.49998Z"
                    fill="#349A37" />
            </svg>
        </div>
        <!--8th section -->
        <div
            class="newsShaddow flex flex-row justify-between py-6 items-center w-full text-[15px] sm:text-lg font-FlatBold text-right text-[#101426]   bg-white md:px-1 xl:px-4">
            <p class="max-w-[475px] ">
                تقديم الدعم النفسي والمعنوي والمادي والاقتصادي والحقوقي القضائي للمقدسيين ....s
            </p>
            <svg class="hidden sm:block" width="11" height="17" viewBox="0 0 11 17" fill="none"
                xmlns="http://www.w3.org/2000/svg">
                <path
                    d="M0.654771 8.49998C0.654771 8.19531 0.771103 7.89067 1.00328 7.65839L8.31297 0.348771C8.77795 -0.116216 9.53185 -0.116216 9.99665 0.348771C10.4614 0.81357 10.4614 1.56732 9.99665 2.03234L3.52863 8.49998L9.99642 14.9677C10.4612 15.4326 10.4612 16.1863 9.99642 16.6511C9.53162 17.1163 8.77773 17.1163 8.31274 16.6511L1.00305 9.34158C0.770839 9.10918 0.654771 8.80454 0.654771 8.49998Z"
                    fill="#349A37" />
            </svg>
        </div>
        <!--9th section -->
        <div
            class="newsShaddow flex flex-row justify-between py-6 items-center w-full text-[15px] sm:text-lg font-FlatBold text-right text-[#101426]   bg-white md:px-1 xl:px-4">
            <p class="max-w-[475px] ">
                إبقاء قضية القدس والمسجد الأقصى حية في الوعي العام العربي والإسلامي ....
            </p>
            <svg class="hidden sm:block" width="11" height="17" viewBox="0 0 11 17" fill="none"
                xmlns="http://www.w3.org/2000/svg">
                <path
                    d="M0.654771 8.49998C0.654771 8.19531 0.771103 7.89067 1.00328 7.65839L8.31297 0.348771C8.77795 -0.116216 9.53185 -0.116216 9.99665 0.348771C10.4614 0.81357 10.4614 1.56732 9.99665 2.03234L3.52863 8.49998L9.99642 14.9677C10.4612 15.4326 10.4612 16.1863 9.99642 16.6511C9.53162 17.1163 8.77773 17.1163 8.31274 16.6511L1.00305 9.34158C0.770839 9.10918 0.654771 8.80454 0.654771 8.49998Z"
                    fill="#349A37" />
            </svg>
        </div>
        <!--10th section -->
        <div
            class="newsShaddow flex flex-row justify-between py-6 items-center w-full text-[15px] sm:text-lg font-FlatBold text-right text-[#101426]   bg-white md:px-1 xl:px-4">
            <p class="max-w-[475px] ">
                . تجهيز دراسات ونشرات وأبحاث ومؤتمرات ومنتديات ومحاضرات وورش عمل للأجيال.....
            </p>
            <svg class="hidden sm:block" width="11" height="17" viewBox="0 0 11 17" fill="none"
                xmlns="http://www.w3.org/2000/svg">
                <path
                    d="M0.654771 8.49998C0.654771 8.19531 0.771103 7.89067 1.00328 7.65839L8.31297 0.348771C8.77795 -0.116216 9.53185 -0.116216 9.99665 0.348771C10.4614 0.81357 10.4614 1.56732 9.99665 2.03234L3.52863 8.49998L9.99642 14.9677C10.4612 15.4326 10.4612 16.1863 9.99642 16.6511C9.53162 17.1163 8.77773 17.1163 8.31274 16.6511L1.00305 9.34158C0.770839 9.10918 0.654771 8.80454 0.654771 8.49998Z"
                    fill="#349A37" />
            </svg>
        </div>
        <!--11th section -->
        <div
            class="newsShaddow flex flex-row justify-between py-6 items-center w-full text-[15px] sm:text-lg font-FlatBold text-right text-[#101426]   bg-white md:px-1 xl:px-4">
            <p class="max-w-[475px] ">
                . تجهيز دراسات ونشرات وأبحاث ومؤتمرات ومنتديات ومحاضرات وورش عمل للأجيال والشرائح المختلفة.
            </p>
            <svg class="hidden sm:block" width="11" height="17" viewBox="0 0 11 17" fill="none"
                xmlns="http://www.w3.org/2000/svg">
                <path
                    d="M0.654771 8.49998C0.654771 8.19531 0.771103 7.89067 1.00328 7.65839L8.31297 0.348771C8.77795 -0.116216 9.53185 -0.116216 9.99665 0.348771C10.4614 0.81357 10.4614 1.56732 9.99665 2.03234L3.52863 8.49998L9.99642 14.9677C10.4612 15.4326 10.4612 16.1863 9.99642 16.6511C9.53162 17.1163 8.77773 17.1163 8.31274 16.6511L1.00305 9.34158C0.770839 9.10918 0.654771 8.80454 0.654771 8.49998Z"
                    fill="#349A37" />
            </svg>
        </div>
        <!--12th section -->
        <div
            class="newsShaddow flex flex-row justify-between py-6 items-center w-full text-[15px] sm:text-lg font-FlatBold text-right text-[#101426]   bg-white md:px-1 xl:px-4">
            <p class="max-w-[475px] ">
                إدخال الممتلكات الوقفية المصادرة كمركب في المطالبة الفلسطينية لإرجاعها إلى حضن .....
            </p>
            <svg class="hidden sm:block" width="11" height="17" viewBox="0 0 11 17" fill="none"
                xmlns="http://www.w3.org/2000/svg">
                <path
                    d="M0.654771 8.49998C0.654771 8.19531 0.771103 7.89067 1.00328 7.65839L8.31297 0.348771C8.77795 -0.116216 9.53185 -0.116216 9.99665 0.348771C10.4614 0.81357 10.4614 1.56732 9.99665 2.03234L3.52863 8.49998L9.99642 14.9677C10.4612 15.4326 10.4612 16.1863 9.99642 16.6511C9.53162 17.1163 8.77773 17.1163 8.31274 16.6511L1.00305 9.34158C0.770839 9.10918 0.654771 8.80454 0.654771 8.49998Z"
                    fill="#349A37" />
            </svg>
        </div>

    </div>
</div>
</div>
