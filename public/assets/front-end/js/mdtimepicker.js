...
(function (factory) {
	...
	var Time = function (hour, minute) {
		...
		this.invert = function () {
			if (this.getPeriod() === 'ص') this.setHour(this.getHour() + 12);
		...
		this.getPeriod = function () { return this.hour < 12 ? 'ص' : 'م'; };
	};
	...
	var MDTimePicker = function (el, config) {
		...
		this.timepicker = {
			...
			clockHolder: {
				...
				buttonsHolder: {
					...
					btnOk: $('<span class="mdtp__button ok">تم</span>'),
					btnCancel: $('<span class="mdtp__button cancel">إلغاء</span>')
				}
			}
		};
		...
		picker.clockHolder.am.click(function () { if (_.selected.getPeriod() !== 'ص') _.setPeriod('ص'); });
	...
	MDTimePicker.prototype = {
				...
				hour.find('span').click(function () {
					var _hour = parseInt($(this).parent().data('hour')),
						...
						_value = _.config.is24hour ? _hour :
							(_hour + ((_selectedT === 'م' && _hour < 12) || (_selectedT === 'ص' && _hour === 12) ? 12 : 0)) % 24,
		...
		setPeriod: function (period) {
			...
			this.timepicker.clockHolder.am[_period === 'ص' ? 'addClass' : 'removeClass']('active');
		...
		parseTime: function (time, tf) {
			...
			// Parse t (am/pm)
			if (is24Hour) t = parseInt(hour) > 11 ? (tLength > 1 ? 'م' : 'pm') : (tLength > 1 ? 'ص' : 'am');
